...
export const handler = async (
	...
	for (const key of houses) {
		...
		const results = RETRIEVE_FROM_API ? 
			await retrieveDataFromApi(s3Client, config.aws.s3Bucket, house, apiFrom, apiTo) : 
			await retrieveDataFromS3(s3Client, config.aws.s3Bucket, house, s3Year, s3Month);	
		
		console.log(`final total results for house of ${house} are ${results.totalResults}`);
		await appendToGoogleSheet(results, config.auth.credentials, config.sheetId, house, apiFrom);
	}
