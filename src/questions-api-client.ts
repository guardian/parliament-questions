...
const getQueryParams = (
	...
	skip: number,
	house: House,
	questionStatus: QuestionsQueryStatus,
) => {
	const take = '50';
	const fromStr = from.format('YYYY-MM-DD');
	const toStr = to.format('YYYY-MM-DD');
	const queryParams =
		questionStatus === QuestionsQueryStatus.Questions
			? new URLSearchParams({
...
export const getQuestions = async (
	...
	house: House,
	questionStatus: QuestionsQueryStatus,
	results: Questions | undefined = undefined,
): Promise<Questions> => {
	const url = `https://questions-statements-api.parliament.uk/api/writtenquestions/questions?`;
